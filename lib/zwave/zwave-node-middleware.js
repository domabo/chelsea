...
ZwaveNodeSerialApi.prototype.invoke = function (context, next) {
    ...
    switch (context[ZWAVE.CommandClass]) {
        case PROTOCOL.CommandClass.ZwaveplusInfo:
            switch (context[ZWAVE.Command]) {
                case PROTOCOL.ZwaveplusInfoCommand.Report:

                    node[ZWAVE.PlusVersion] = payload[0];

                    node[ZWAVE.PlusRole] = PROTOCOL.ZwavePlusRoleType.properties[payload[1]] || {};
                    node[ZWAVE.PlusRole].value = payload[1];

                    node[ZWAVE.PlusNodeType] = PROTOCOL.ZwavePlusNodeType.properties[payload[2]] || {};
                    node[ZWAVE.PlusNodeType].value = payload[2];

                    var plusInstallerIcon = (payload[3] << 8) | payload[4];
                    node[ZWAVE.PlusInstallerIcon] = PROTOCOL.ZwavePlusIcon.properties[plusInstallerIcon] || {};
                    node[ZWAVE.PlusInstallerIcon].id = toHex(plusInstallerIcon);
                    node[ZWAVE.PlusInstallerIcon].value = plusInstallerIcon;

                    var plusDeviceType = (payload[5] << 8) | payload[6];
                    node[ZWAVE.PlusDeviceType] = PROTOCOL.ZwavePlusCommandClass.properties[plusDeviceType] || {};
                    node[ZWAVE.PlusDeviceType].value = plusDeviceType;
                    break;
            }
            break;
